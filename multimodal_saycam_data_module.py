...
import sys
sys.path.append("/home/p97779fl/Datasets/multimodal-baby")
...
DATA_DIR = Path("/home/p97779fl/Datasets/saycam-s")
GSHEETS_CREDENTIALS_FILENAME = DATA_DIR / "credentials.json"
TRANSCRIPT_LINKS_FILENAME = DATA_DIR / "SAYCam_transcript_links.csv"
TRANSCRIPTS_DIRNAME = DATA_DIR / "transcripts/filtered"
PREPROCESSED_TRANSCRIPTS_DIRNAME = DATA_DIR / "transcripts/preprocessed"
RAW_VIDEO_DIRNAME = "/home/p97779fl/Datasets/saycam-s/S_multimodal/S_videos_annotations/S_videos/"
LABELED_S_DIRNAME = "/home/p97779fl/Datasets/saycam-s/labeled_s"
FILTERED_LABELED_S_DIRNAME = "/home/p97779fl/Datasets/saycam-s/S_multimodal/S_clean_labeled_data_clip_filtered"
EXTRACTED_FRAMES_DIRNAME = DATA_DIR / 'frames' / "train_5fps"
# EVAL_FRAMES_DIRNAME = DATA_DIR / "eval"
# FILTERED_EVAL_FRAMES_DIRNAME = DATA_DIR / "eval_filtered"
EVAL_FRAMES_DIRNAME = DATA_DIR / "S_multimodal"/"eval"
FILTERED_EVAL_FRAMES_DIRNAME = DATA_DIR / "S_multimodal"/"eval_filtered"
MANUAL_FILTERED_EVAL_FRAMES_DIRNAME = DATA_DIR / "S_multimodal" / "eval_manual_filtered"
...
EVAL_DEV_METADATA_FILENAME = DATA_DIR / "S_multimodal" / "eval_dev.json"
EVAL_TEST_METADATA_FILENAME = DATA_DIR / "S_multimodal" / "eval_test.json"
FILTERED_EVAL_DEV_METADATA_FILENAME = DATA_DIR / "S_multimodal" /"eval_filtered_dev.json"
FILTERED_EVAL_TEST_METADATA_FILENAME = DATA_DIR / "S_multimodal" /"eval_filtered_test.json"
MANUAL_FILTERED_EVAL_TEST_METADATA_FILENAME = DATA_DIR / "S_multimodal" /"eval_manual_filtered_test.json"
VOCAB_FILENAME = DATA_DIR / "S_multimodal" / "vocab.json"
...
def _download_transcripts():
    ...
    print("Downloading SAYCam transcripts from Google Drive + Sheets")

    # 创建存放目录
    os.makedirs(TRANSCRIPTS_DIRNAME, exist_ok=True)
    ...
    for i, url in enumerate(urls):
        if i > 433:
            print(f"\n📥 Downloading SAYCam transcript {i+1}/{len(urls)}:\n{url}")
            ...
            try:
                if "drive.google.com/file/d/" in url:
                    ...
                    file.FetchMetadata(fields="id, title, mimeType, shortcutDetails")
    
                    # 如果是快捷方式（shortcut），跟踪真正目标 ID
                    if "shortcutDetails" in file:
                        file_id = file["shortcutDetails"]["targetId"]
                        file = drive.CreateFile({'id': file_id})
                        file.FetchMetadata(fields="id, title, mimeType")
    
                    # 构造 Google Sheets 链接
                    sheets_url = f"https://docs.google.com/spreadsheets/d/{file_id}/edit"
    
                    # 用 gsheets 打开并导出每个标签页
                    s = sheets.get(sheets_url)
                    title_parts = s.title.split("_")
                    title = "_".join(title_parts[:3]) if len(title_parts) >= 3 else s.title
    
                    for j in range(1, len(s.sheets)):
                        try:
                            sheet_df = s.sheets[j].to_frame()
                            sheet_title = s.sheets[j].title.replace(" ", "_")
                            filename = f"{TRANSCRIPTS_DIRNAME}/{title}_{sheet_title}.csv"
                            ...
                            if os.path.exists(filename):
                                ...
                                time.sleep(5)  # 防止 API 限流·
                        except pd.errors.ParserError:
                            print(f"⚠️ Skipped sheet: {s.sheets[j].title}")
                            continue
    
                else:
                    print(f"❌ URL 格式不正确（非 Google Drive 文件链接）：{url}")
...
def _preprocess_transcripts():
    ...
    if os.path.exists(PREPROCESSED_TRANSCRIPTS_DIRNAME):
        ...
        # allowed_speakers = ["S", "Sam", "Alice", "s", "sam", "child", "Alex"]
        ...
        for transcript_idx, transcript_filename in enumerate(transcripts):
            ...
            if video_filename.stem != transcript_filename.stem:
                print(video_filename.stem, transcript_filename.stem)
                video_filename = transcript_filename.with_suffix(".mp4")
            ...
            for transcript_row_idx, row in transcript.iterrows():
                ...
                if pd.isnull(start_timestamp) or pd.isnull(end_timestamp) or start_timestamp > end_timestamp or speaker not in allowed_speakers:
                    continue
        ...
        with torch.no_grad():  # ✅ 关闭梯度追踪
            for i, eval_category in enumerate(eval_categories):
                ...
                for frame in frames:
                    ...
                    del image, image_features, logits_per_text
...
def _create_eval_metadata():
    ...
    if os.path.exists(EVAL_DEV_METADATA_FILENAME) and os.path.exists(EVAL_TEST_METADATA_FILENAME):
        ...
        eval_categories = sorted(os.listdir("/home/p97779fl/Datasets/saycam-s/labeled_s"))
